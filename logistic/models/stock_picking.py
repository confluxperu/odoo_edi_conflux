...
class StockPicking(models.Model):
	...
	def _prepare_despatch(self):
		...
		#ir_sequence = self.env['ir.sequence'].search([('type','=','general'),('code','=like','T%')])
		#if not ir_sequence:
		#	raise UserError('Journals not found for despatchs')

		origin_address_id = False
		delivery_address_id = False
		warehouse_id = False
		if picking.picking_type_id.code=='incoming':
			origin_address_id = picking.company_id.partner_id.id
